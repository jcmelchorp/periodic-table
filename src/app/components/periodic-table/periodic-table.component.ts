...
import { Atom, HighlightState } from 'src/app/shared';
...
export class PeriodicTableComponent implements OnInit, OnChanges {
  ...
  atoms: Atom[];
  metalClass: HighlightState | undefined;
  currentAtom: Atom;
  ...
  constructor(private http: HttpClient) {
    this.colHeader = Array(MAX_COL_INDEX)
      ...
      .map((v, i) => ({
        index: i + 1,
        description: i === 0 ? 'Alcalinos' : i === 1 ? 'Alcalinotérreos' : i === 12 ? 'Térreos' : i === 13 ? 'Carbonoideos' : i === 14 ? 'Nitrogenoides' : i === 15 ? 'Anfígenos o Calcógenos' : i === 16 ? 'Halógenos' : '',
        selected: false,
      }));
  ...
  ngOnInit() {
    ...
    this.atoms$ = combineLatest(
      this.http.get<Atom[]>('./assets/data/periodic-table.json').pipe(startWith([] as Atom[])),
      this.headerMove$
    ).pipe(
